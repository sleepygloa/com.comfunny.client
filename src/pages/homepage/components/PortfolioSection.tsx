...
const ProjectDetailModal: React.FC<{ project: any, onClose: () => void }> = ({ project, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4 bg-black/60 backdrop-blur-sm transition-opacity duration-300" onClick={handleBackdropClick}>
                ...
                <div className="overflow-y-auto p-4 md:p-8">
                    <div className="mb-8">
                        <div className="flex flex-wrap items-center gap-3 mb-4">
                            <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold uppercase tracking-wide">{project.category}</span>
                            <span className="text-slate-500 text-xs md:text-sm font-medium flex items-center">
                                <span className="w-1 h-1 bg-slate-300 rounded-full mx-2"></span>{project.date}
                            </span>
                             <span className="text-slate-500 text-xs md:text-sm font-medium flex items-center">
                                <span className="w-1 h-1 bg-slate-300 rounded-full mx-2"></span>{project.role}
                            </span>
                        </div>
                        <h1 className="text-2xl md:text-4xl font-black text-slate-900 mb-6 leading-tight">{project.title}</h1>
                        <p className="text-base md:text-lg text-slate-600 leading-relaxed font-light">{project.fullDesc}</p>
                    </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        <div className="md:col-span-2 space-y-8">
                            <div>
                                ...
                                <p className="text-slate-600 leading-relaxed text-sm md:text-base">{project.challenge}</p>
                            </div>
                            <div>
                                <h3 className="text-lg font-bold text-slate-900 mb-3 border-l-4 border-blue-500 pl-3">The Solution</h3>
                                <p className="text-slate-600 leading-relaxed text-sm md:text-base">{project.solution}</p>
                            </div>
                        </div>
                            ...
                            <div className="bg-slate-50 p-5 rounded-xl border border-slate-100">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {project.tech.map((t: string) => (
                                        <span key={t} className="px-2 py-1 bg-white border border-slate-200 rounded text-xs font-bold text-slate-600">{t}</span>
                                    ))}
                                </div>
                            </div>
                            <div className="bg-slate-50 p-5 rounded-xl border border-slate-100">
                                ...
                                <div className="space-y-2">
                                    <a href={project.links.demo} target="_blank" rel="noreferrer" className="flex items-center justify-between w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm text-sm font-bold opacity-50 cursor-not-allowed" onClick={(e) => e.preventDefault()}>
                                        Live Demo (Private) <IconExternalLink className="w-4 h-4" />
                                    </a>
                                    <a href={project.links.github} target="_blank" rel="noreferrer" className="flex items-center justify-between w-full px-4 py-2 bg-white text-slate-700 border border-slate-200 rounded-lg hover:border-slate-400 transition-colors text-sm font-bold opacity-50 cursor-not-allowed" onClick={(e) => e.preventDefault()}>
                                        GitHub Repo (Private) <IconGithub className="w-4 h-4" />
                                    </a>
                                </div>
                            </div>
                        </div>
    ...
    return (
        <section id="portfolio" className="py-16 md:py-24 bg-white min-h-[500px] transition-all">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-lg">
                            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`} aria-label="Grid View"><IconGrid className="w-5 h-5" /></button>
                            <button onClick={() => setViewMode('list')} className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`} aria-label="List View"><IconList className="w-5 h-5" /></button>
                        </div>
                    </div>
                </div>
                ...
                {filteredProjects.length > 0 ? (
                    <>
                        {viewMode === 'grid' && (
                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                                {filteredProjects.map((project) => (
                                    <div key={project.id} onClick={() => handleProjectClick(project.id)} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 hover:border-blue-100 hover:-translate-y-2 cursor-pointer">
                                        <div className="relative h-48 md:h-60 overflow-hidden">
                                            ...
                                            <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-6">
                                                <span className="text-white font-bold text-sm flex items-center">View Details <IconArrowDown className="ml-2 w-4 h-4 -rotate-90" /></span>
                                            </div>
                                        </div>
                                        <div className="p-6 md:p-8">
                                            <span className="inline-block px-2 py-1 bg-slate-50 text-blue-600 text-[10px] md:text-xs font-bold rounded-full uppercase tracking-wide mb-3">{project.category}</span>
                                            <h3 className="text-lg md:text-xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors mb-2">{project.title}</h3>
                                            <p className="text-slate-600 text-xs md:text-sm leading-relaxed mb-4 line-clamp-2">{project.shortDesc}</p>
                                            <div className="flex flex-wrap gap-1.5 pt-4 border-t border-slate-50">
                                                {project.tech.map((t) => (
                                                    <span key={t} className={`px-2 py-1 text-[10px] md:text-xs rounded-full font-bold transition-all ${selectedTech === t ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-600'}`}>{t}</span>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
                        ...
                        {viewMode === 'list' && (
                            <div className="flex flex-col gap-4">
                                {filteredProjects.map((project) => (
                                    <div key={project.id} onClick={() => handleProjectClick(project.id)} className="group flex flex-col md:flex-row bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-slate-100 hover:border-blue-200 cursor-pointer">
                                        ...
                                        <div className="p-6 flex flex-col justify-center flex-grow">
                                            <div className="flex justify-between items-start mb-2">
                                                <span className="inline-block px-2 py-0.5 bg-blue-50 text-blue-600 text-[10px] font-bold rounded-md uppercase tracking-wide">{project.category}</span>
                                            ...
                                            <p className="text-slate-600 text-sm leading-relaxed mb-4 line-clamp-2 md:line-clamp-1">{project.shortDesc}</p>
                    ...
                    <div className="text-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                        <div className="inline-block p-4 bg-white rounded-full mb-4 shadow-sm"><IconCode className="w-8 h-8 text-slate-300" /></div>
            ...
            {activeProjectId && selectedProjectData && (
                <ProjectDetailModal project={selectedProjectData} onClose={() => setActiveProjectId(null)} />
            )}
        </section>
    );
}
