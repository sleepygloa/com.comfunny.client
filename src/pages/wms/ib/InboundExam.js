...
export default function InboundExam() {
  ...
  // 데이터 리스트 상태 관리
  const [dataList, setDataList] = useState([]); // 메인 데이터 리스트
  const [dataDtlList, setDataDtlList] = useState([]); // 상세 데이터 리스트
        ...
        const updatedRows = dataDtlList.map(row => {
          if (row.id === id) {
            const newRow = {
              ...
              [field]: Number(value),
              // 총 검수 수량 계산
              examTotQty: (field === 'examBoxQty' ? Number(value) : row.examBoxQty) * row.pkqty 
                          + (field === 'examEaQty' ? Number(value) : row.examEaQty),
            };
            return newRow;
          }
          return row;
  ...
  return (
    <>
      ...
      <ComDeGrid
        ...
        searchBarChildren={
          <FieldRow>
            <SchTextField id="ibNo" label='입고번호/명' div="4" onChange={onChangeSearch} onKeyDown={onKeyDown} />  
            <SchDateField id="ibPlanYmd" label='입고예정일' div="4" selected={schValues.ibPlanYmd} onChange={onChangeSearch} />    
          </FieldRow>
        }
        title={"Inbound List"}
        dataList={dataList}
