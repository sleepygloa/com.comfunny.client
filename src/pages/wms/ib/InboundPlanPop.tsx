...
// Popup
// InboundPlanItemPop 컴포넌트 import (존재한다고 가정)
...
export default function InboundPlanPop(props: InboundPlanPopProps) {
  ...
  const [dataList, setDataList] = useState<InboundPlanDetailData[]>([]);

  // 콤보박스
  const [dcCmb, setDcCmb] = useState<any[]>([]);
  const [clientCmb, setClientCmb] = useState<any[]>([]);
  const [supplierCmb, setSupplierCmb] = useState<any[]>([]);
  const [ibGbnCdCmb, setIbGbnCdCmb] = useState<any[]>([]);
  const [ibProgStCdCmb, setIbProgStCdCmb] = useState<any[]>([]);
  ...
  const fnSearchClient = async () => {
    await client.post(`${PRO_URL}/selectClientCmbList`, null)
      .then(res => setClientCmb(gvSetDropdownData(res.data)))
      .catch(error => console.log('error = ' + error));
  }
  
  const fnSearchSupplier = async (data: any) => {
    await client.post(`${PRO_URL}/selectSupplierCmbList`, data)
      .then(res => setSupplierCmb(gvSetDropdownData(res.data)))
      .catch(error => console.log('error = ' + error));
  }
