...
import { 
  ...
  gvGridFieldParsePhoneNumber,
  gvGridFieldInputPhoneNumber,
  gvGridFieldFormatFaxNumber,
  gvGridFieldParseFaxNumber,
  gvGridFieldInputFaxNumber,
...
export default function Area() {
  ...
  const [selRowId, setSelRowId] = useState(-1); // 선택된 행 ID
  const [dataList, setDataList] = useState([]); // 데이터 리스트
  const [useYnCmb, setUseYnCmb] = useState([]); // 사용 여부 콤보박스 옵션
  const [dcCmb, setDcCmb] = useState([]); // 물류 센터 콤보박스 옵션
  const [keepTempGbnCmb, setKeepTempGbnCmb] = useState([]); // 보관 온도 구분 콤보박스 옵션
  const [schValues, setSchValues] = useState({ codeCd: "" }); // 검색 조건 값

  // 초기 데이터 값 설정
  const initData = {
    id: dataList.length + 1, // 새 데이터의 ID
    bizCd: '',
    dcCd: "",
    areaCd: "",
    areaNm: "",
  ...
  const [values, setValues] = useState(initData); // 현재 선택된 값

  // DataGrid의 컬럼 정의
  const columns = [
    { field: "id", headerName: "ID", align: "center", width: 20 },
    { field: "dcCd", headerName: "물류창고", editable: true, width: 150, align: "center", type: "singleSelect", valueFormatter: gvGridDropdownDisLabel, valueOptions: dcCmb },
    ...
    { field: "keepTempeGbnCd", headerName: "보관온도구분", editable: true, align: "center", type: "singleSelect", valueFormatter: gvGridDropdownDisLabel, valueOptions: keepTempGbnCmb },
  ...
  // 데이터 리스트에 새 항목 추가
  const onClickAdd = () => setDataList(prevDataList => [...prevDataList, { ...initData, id: prevDataList.length + 1 }]);
