...
var warehouse = function( container ) {
	...
	(function( warehouse ) {
	    ...
	    var floor_1 = new THREE.GridHelper( 1360, 34 );
	    floor_1.position.x = -1360;
		warehouse.scene.add( floor_1 );

	    var floor_2 = new THREE.GridHelper( 1360, 34 );
		warehouse.scene.add( floor_2 );

	    var floor_3 = new THREE.GridHelper( 1360, 34 );
	    floor_3.position.x = 1360;
		warehouse.scene.add( floor_3 );

		var floor_geometry = new THREE.PlaneGeometry(4080, 1360);
		...
		// 외벽 PlaneGeometry ==============================================================================
		var wall_geometry = new THREE.PlaneGeometry(1360, 200);
	    ...
	    wall_object.position.set( 2040, 100, 0 );
	    warehouse.scene.add( wall_object );

		wall_geometry = new THREE.PlaneGeometry(1360, 200);
		...
		warehouse.cameraControls = function () {
            ...
            warehouse.controls.target.set(0, 0, 800);	// 포커스변경
				...
				if( intersects.length > 0 ) {
					...
					if( warehouse.INTERSECTED != intersects[ 0 ].object ) {
						...
						if( intersects[ 0 ].object.type == 'stok' || intersects[ 0 ].object.type == 'stat' ){
							...
							warehouse.INTERSECTED.callback( location.stok_arr );
							
							var max_diameter = 0;
							for(var i=0 ; i<location.stok_arr.length ; i++){
								if( max_diameter < parseInt(location.stok_arr[i].diameter) ) max_diameter = parseInt(location.stok_arr[i].diameter);
							}
							//=============================================================
							// 재고객체의 경우 material array 를 사용하면서 성능향상을 위해 전역변수로 설정하여 material 를 부분적으로 컨트롤 할 수 없다
							// 그러기에 선택 객체와 동일한 cover 객체를 생성하여 선택효과를 준다.
							var cover_geometry = new THREE.BoxBufferGeometry( loc_info.horizontal + 4, (max_diameter * 2) + 4, loc_info.vertical + 4 );
							...
							cover_object.position.y = max_diameter;
 ...
 * zone 객체생성
 ******************************************************************************************************/
var center_x = 2040;	// 중심축좌표
var center_z = 520;

var zone_arr = [

	{ zone: 'B1', zonenm: '커터#1', horizontal: 400, vertical: 800, position_x: 120 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  ,	{ zone: 'B2', zonenm: '커터#2', horizontal: 320, vertical: 800, position_x: 560 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  ,	{ zone: 'B3', zonenm: '커터#3', horizontal: 320, vertical: 800, position_x: 960 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  ,	{ zone: 'B4', zonenm: '커터#4', horizontal: 320, vertical: 800, position_x: 1320 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  , { zone: 'B5', zonenm: '커터#5', horizontal: 400, vertical: 800, position_x: 1720 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  ,	{ zone: 'B6', zonenm: '커터#6', horizontal: 400, vertical: 800, position_x: 2160 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  ,	{ zone: 'B7', zonenm: '커터#7', horizontal: 400, vertical: 800, position_x: 2640 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  ,	{ zone: 'B8', zonenm: '커터#8', horizontal: 400, vertical: 800, position_x: 3080 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
  ,	{ zone: 'B9', zonenm: '커터#9', horizontal: 400, vertical: 800, position_x: 3560 - center_x, position_z: 120 - center_z, color: 0xf6f6f6 }
]

var zoneGeometry = function( zone_arr_idx, scene ) {

	(function( zoneGeometry ) {

		zoneGeometry.zone = zone_arr[zone_arr_idx]; 
		...
		zoneGeometry.draw = function () {
		    ...
		    zone_object.name = zoneGeometry.zone.zone;
			
		    //scene.add(zone_object);
		    
		    // ZONE Label 추가
			var rotation = new THREE.Euler( -0.5 * Math.PI, 0, 0 );
			zone_position_x = zone_position_x - ( zoneGeometry.zone.horizontal / 2 ) + 100;
			zone_position_z = zone_position_z + ( zoneGeometry.zone.vertical / 2 ) + 100;

			addLabel( zone_arr[zone_arr_idx].zonenm, new THREE.Vector3( zone_position_x, 0, zone_position_z ), rotation, 40, 0xeaeaea );
		}
...
var locGeometry = function( prameter ) {
	...
	(function( locGeometry ) {
	    ...
	    var loc_position_x = prameter.start_x + (loc_info.horizontal / 2) + (loc_info.padding / 2);
	    ...
	    locGeometry.draw = function () {
			...
			loc_object.position.x = loc_position_x + ( loc_info.horizontal + loc_info.padding ) * (x - 1);
			loc_object.position.y = loc_position_y + ( loc_info.height + loc_info.padding ) * y;
			loc_object.position.z = loc_position_z;
			...
			label_position_x = loc_object.position.x - ( loc_info.horizontal / 2 ) + 10;
			label_position_z = loc_object.position.z + ( loc_info.vertical / 2 ) + 10;

			addLabel( prameter.locnm, new THREE.Vector3( label_position_x, 0, label_position_z ), rotation, 20, 0x0100ff );
		}
